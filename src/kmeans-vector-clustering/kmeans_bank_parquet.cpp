...
int main(int argc, char *argv[])
{
    ...
    // Tiling parameters
    const size_t tile_size = std::min(50000UL, std::max(10000UL, N / 8));
    const size_t num_tiles = (N + tile_size - 1) / tile_size;
    
    std::cout << "[TILING] Tile size: " << tile_size << ", Number of tiles: " << num_tiles << std::endl;

    // Copy data to pinned memory first, then to GPU using copy_stream
    ...
    cudaEventRecord(up_evt, copy_stream); // H2D 끝나면 완료로 기록
    ...
    // CORNER TURNING: Transpose centroids for better memory access pattern
    ...
    {
        // Wait for previous iteration's data transfer to complete
        if(it > 1) cudaEventSynchronize(d2h_done_evt[pong]);
