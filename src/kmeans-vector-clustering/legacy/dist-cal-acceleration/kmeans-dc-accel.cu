...
__global__ void kmeans_labeling_kernel(float *d_samples, int *d_clusterIndices, float *d_clusterCenters, int N, int K, int DIM, NormType normType)
{
    ...
    for (int k = 0; k < K; ++k)
    {
        ...
        float distance = get_distance(curPoint, curCentroid, DIM, normType);
...
void launch_kmeans_labeling(float *d_samples, int *d_clusterIndices, float *d_clusterCenters, int N, int TPB, int K, int DIM, NormType normType)
{
    kmeans_labeling_kernel<<<(N + TPB - 1) / TPB, TPB>>>(d_samples, d_clusterIndices, d_clusterCenters, N, K, DIM, normType);
}
