    ...
    )
{
    ...
    cublasSgemm(cublasHandle, CUBLAS_OP_N, CUBLAS_OP_T, Dim, Dim, N, &alpha, d_X, N, d_X, N, &beta, d_Cov, Dim);
    ...
    std::cout << "--------------------------------" << std::endl;
    std::cout << "h_eigenVal의 Eigenvalues: ";
    for (float val : h_eigenVal) {
        std::cout << val << " ";
    }
    std::cout << std::endl;

    std::vector<float> h_Cov(Dim * Dim);
    cudaMemcpy(h_Cov.data(), d_Cov, Dim * Dim * sizeof(float), cudaMemcpyDeviceToHost);

    std::cout << "추출된 eigenvector (h_eigenVec) 값:" << std::endl;
    for (int i = 0; i < Dim * reducedDim; ++i) {
        std::cout << h_eigenVec[i] << " ";
    }
    std::cout << std::endl;

    std::cout << "d_Cov의 마지막 두 열 (예상 eigenvector):" << std::endl;
    for (int col = Dim - reducedDim; col < Dim; ++col) {
        for (int row = 0; row < Dim; ++row)  {
            std::cout << h_Cov[row + col * Dim] << " ";
        }
        std::cout << std::endl;
    }
    std::cout << "--------------------------------" << std::endl;
    ...
    for(int i = 0; i < reducedDim; i++){
        std::cout << "Eigenvalue " << i << ": " << sorted_eigVals[i] << std::endl;
        top_variance += sorted_eigVals[i];
    }
    ...
    int N, int Dim, int reducedDim)
{
    ...
    std::cout << "================================" << std::endl;
    std::cout << "h_data의 첫 5개 좌표값: \n";
    for (int i = 0; i < 5; ++i) {
        for (int d = 0; d < Dim; ++d) {
            std::cout << h_data[i * Dim + d] << " ";
        }
        std::cout << std::endl;
    }
    std::cout << "h_data의 첫 5개 좌표값 (정규화 후): \n";
    for (int i = 0; i < 5; ++i) {
        for (int d = 0; d < Dim; ++d) {
            std::cout << tmp_h_data[i * Dim + d] << " ";
        }
        std::cout << std::endl;
    }
    ...
    pca_cuSOLVER_SVD(tmp_h_data, h_reducedData, h_eigenVec, N, Dim, reducedDim, cublasHandle, cusolverHandle);
...
)
{
    ...
    float* d_stdVec = nullptr;
    cudaMalloc(&d_meanVec, Dim * sizeof(float));
    cudaMalloc(&d_stdVec, Dim * sizeof(float));
    cudaMemcpy(d_meanVec, h_meanVec.data(), Dim * sizeof(float), cudaMemcpyHostToDevice);
    cudaMemcpy(d_stdVec, h_stdVec.data(), Dim * sizeof(float), cudaMemcpyHostToDevice);

    int TPB = 128;
    launch_denormalize(d_out, d_meanVec, d_stdVec, N, Dim, TPB);
...
int main(int argc, char *argv[])
{
    ...
    file_write_sample(h_samples, h_clusterIndices, N, dimension, "samples_2.txt");
    file_write_sample(h_reducedSamples, h_clusterIndices, N, reducedDim, "samples_reduced_2.txt");
#endif
