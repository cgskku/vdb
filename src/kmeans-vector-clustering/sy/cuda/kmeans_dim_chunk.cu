    ...
    int N, int K, int DIM, int chunkSize) 
{
    ...
    // Simplified implementation: calculate distance directly without shared memory
    // This is for debugging purposes to isolate the chunk logic issue
    ...
    for (int k = 0; k < K; ++k) {
        ...
        // Calculate distance in chunks to test chunk logic
        for (int offset = 0; offset < DIM; offset += chunkSize) {
            int curChunkSize = min(chunkSize, DIM - offset);
            
            for (int d = 0; d < curChunkSize; ++d) {
                float diff = d_samples[globalThreadIndex * DIM + offset + d] - 
                           d_clusterCenters[k * DIM + offset + d];
                distance += diff * diff;
            }
        }
...
// Basic implementation functions for testing
__global__ void kmeans_labeling_basic_kernel(
    float* d_samples,
    int* d_clusterIndices,
    float* d_clusterCenters,
    int N, int K, int DIM) 
{
    ...
    for (int k = 0; k < K; ++k) {
        float distance = 0.0f;
        for (int d = 0; d < DIM; ++d) {
            float diff = d_samples[globalThreadIndex * DIM + d] - d_clusterCenters[k * DIM + d];
            distance += diff * diff;
        }
        distance = sqrt(distance);
...
__global__ void kmeans_update_centers_basic_kernel(
    float* d_samples,
    int* d_clusterIndices,
    float* d_clusterCenters,
    int* d_clusterSizes,
...
void launch_kmeans_labeling(
    float* d_samples, int* d_clusterIndices, 
    float* d_clusterCenters, 
    int N, int TPB, int K, int DIM) 
{
    kmeans_labeling_basic_kernel<<<(N + TPB - 1) / TPB, TPB>>>(
    ...
    )
{
    ...
    kmeans_update_centers_basic_kernel<<<(N + TPB - 1) / TPB, TPB>>>(
