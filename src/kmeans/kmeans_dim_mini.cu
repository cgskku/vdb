    ...
    int N, int S, int K, int DIM) 
{
    ...
    d_clusterIndices[globalThreadIndex + S] = closestCenterIndex;
}
    ...
    int N, int S, int K, int DIM) 
{
    ...
    const int globalThreadIndex = blockIdx.x * blockDim.x + threadIdx.x;
    const int pointIndex = globalThreadIndex + S;
    const int tid = threadIdx.x;
    ...
    for (int offset = 0; offset < DIM; offset += chunkSize) {
        ...
        if (globalThreadIndex < N) {
            int cluster_id = d_clusterIndices[pointIndex];
...
void launch_kmeans_labeling(float* d_samples, int* d_clusterIndices, float* d_clusterCenters, int N, int S, int TPB, int K, int DIM) {
    ...
    kmeans_labeling_kernel<<<(N + TPB - 1) / TPB, TPB, shared_memory_size>>>(
        d_samples, d_clusterIndices, d_clusterCenters, N, S, K, DIM);
}
...
void launch_kmeans_update_center(float* d_samples, int* d_clusterIndices, float* d_clusterCenters, int* d_clusterSizes, int N, int S, int TPB, int K, int DIM) {
    ...
    kmeans_update_centers_kernel<<<(N + TPB - 1) / TPB, TPB, shared_memory_size>>>(
        d_samples, d_clusterIndices, d_clusterCenters, d_clusterSizes, N, S, K, DIM);
