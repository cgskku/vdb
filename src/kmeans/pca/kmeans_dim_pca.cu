    ...
    int N, int K, int DIM, int chunkSize) 
{
    ...
    for(int offset = 0; offset < DIM; offset += chunkSize){
        ...
        for(int k = 0; k < K; ++k){
            ...
            // (squaredDistance < minDistance) ? minDistance = squaredDistance, closestCenterIndex = k : 0;
        }
        __syncthreads();
    }
    ...
    int N, int K, int DIM, int chunkSize) 
{
    ...
    // (globalThreadIndex < N) ? atomicAdd(&d_clusterSizes[d_clusterIndices[globalThreadIndex]], 1) : 0;
}
...
void launch_extract_top_eigenvectors(
    float* d_Cov, float* d_Vsub,
    int Dim, int reducedDim, int TPB
    )
{
    int blocks = (Dim + TPB - 1) / TPB;
