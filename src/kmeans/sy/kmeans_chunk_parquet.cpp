...
int main(int argc, char *argv[])
{
    ...
    if (argc != 5) 
    {
        std::cout << "Usage: " << argv[0] << " <parquet_file1> <parquet_file2> <K> <MAX_ITER>" << std::endl;
        std::cout << "Example: " << argv[0] << " data1.parquet data2.parquet 10 100" << std::endl;
        return 1;
    }
    ...
    const std::size_t K     = static_cast<std::size_t>(std::atoll(argv[3]));    // K number of clusters
    const int MAX_ITER      = std::atoi(argv[4]);   // 5

    std::cout << "Loading parquet files..." << std::endl;
    ...
    // std::cout << "Final SSE = " << sse << std::endl;
    ...
    std::cout << "Total execution time: " << std::fixed << std::setprecision(3) << elapsed.count() << " ms" << std::endl;
    std::cout << "Average time per iteration: " << std::fixed << std::setprecision(3) << elapsed.count() / final_iteration << " ms" << std::endl;
    if (converged) 
    {
        std::cout << "Time saved: " << std::fixed << std::setprecision(3) << (MAX_ITER - final_iteration) * (elapsed.count() / final_iteration) << " ms" << std::endl;
    }
    std::cout << "=================================" << std::endl;
