...
#define CUBLAS_CHECK(x) do { cublasStatus_t st = (x); if (st != CUBLAS_STATUS_SUCCESS) { \
    std::cerr << "[cuBLAS Error] status=" << st \
...
//////////////////////////////////////////////////////////////////////////////////////////// 
    // flat data 도 pinned memory로 할당
    float* flat_data = nullptr; // pinned
    size_t flat_count = N * (size_t)dimension;
    CUDA_CHECK(cudaMallocHost((void**)&flat_data, flat_count * sizeof(float)));

    for (std::size_t i = 0; i < N; ++i) {
        std::copy(data[i].second.begin(), data[i].second.end(), flat_data + i * (size_t)dimension);
    }
    size_t data_bytes = flat_count * sizeof(float);
    std::cout << "[Info] data size: " << flat_count
              << " (bytes: " << data_bytes << ", MB: " << data_bytes / (1024 * 1024) << ")\n";
...
////////////////////////////////////////////////////////////////////////////////////////////  
    ...
    cudaEvent_t up_evt;
    CUDA_CHECK(cudaEventCreateWithFlags(&up_evt, cudaEventDisableTiming));

    cudaEvent_t ker_done_evt[2], d2h_done_evt[2]; // ping-pong
    for (int p = 0; p < 2; ++p) {
        CUDA_CHECK(cudaEventCreateWithFlags(&ker_done_evt[p],  cudaEventDisableTiming));
        CUDA_CHECK(cudaEventCreateWithFlags(&d2h_done_evt[p],  cudaEventDisableTiming));
    }
 //////////////////////////////////////////////////////////////////////////////////////////// 
    // H2D copy using stream (pinned -> GPU with async)
    CUDA_CHECK(cudaMemcpyAsync(d_db_vectors, flat_data, data_bytes, cudaMemcpyHostToDevice, copy_stream));
    CUDA_CHECK(cudaEventRecord(up_evt, copy_stream)); // H2D 끝나면 완료로 기록
 //////////////////////////////////////////////////////////////////////////////////////////// 

    float* d_row_norm2_all = nullptr;
    float* d_db_T = nullptr;
    CUDA_CHECK(cudaMalloc(&d_row_norm2_all, N * sizeof(float)));
    CUDA_CHECK(cudaMalloc(&d_db_T, N * dimension * sizeof(float)));

    CUDA_CHECK(cudaStreamWaitEvent(k_stream, up_evt, 0));
    launch_compute_row_norm2_all(d_db_vectors, d_row_norm2_all, (int)N, (int)dimension, TPB, k_stream);
    launch_transpose_rowmajor_to_colmajor(d_db_vectors, d_db_T, (int)N, (int)dimension, k_stream);

 //////////////////////////////////////////////////////////////////////////////////////////// 
    cublasHandle_t handle;
    CUBLAS_CHECK(cublasCreate(&handle));
    CUBLAS_CHECK(cublasSetStream(handle, k_stream));
    const float alpha = 1.0, beta = 0.0;
 //////////////////////////////////////////////////////////////////////////////////////////// 

    size_t max_tile_elems = tile_size * (size_t)tile_size;

    float* d_C[2] = {nullptr, nullptr};         // GPU ping-pong
    CUDA_CHECK(cudaMalloc(&d_C[0], max_tile_elems * sizeof(float)));
    CUDA_CHECK(cudaMalloc(&d_C[1], max_tile_elems * sizeof(float)));

    float* h_tile[2] = {nullptr, nullptr};      // Host pinned ping-pong
    CUDA_CHECK(cudaMallocHost((void**)&h_tile[0], max_tile_elems * sizeof(float)));
    CUDA_CHECK(cudaMallocHost((void**)&h_tile[1], max_tile_elems * sizeof(float)));

    // "이미 끝난 상태"로 초기화해놓기 -> 첫 루프에서 안기다리게
    CUDA_CHECK(cudaEventRecord(d2h_done_evt[0], copy_stream));
    CUDA_CHECK(cudaEventRecord(d2h_done_evt[1], copy_stream));
 ...
 //////////////////////////////////////////////////////////////////////////////////////////// 
    ...
    for (size_t tile_j = 0; tile_j < num_tiles; ++tile_j) {
        ...
        for (size_t tile_i = 0; tile_i < num_tiles; ++tile_i) {
            ...
            CUDA_CHECK(cudaStreamWaitEvent(k_stream, d2h_done_evt[p], 0));
            ...
            CUBLAS_CHECK(cublasSgemm(
                ...
                dB, (int)dimension,
                &beta,
                d_C[p], (int)tile_rows   // ldc = tile_rows (col-major)
            ));

            launch_fuse_norms_and_dot_to_dist2_colmajor(d_row2, d_col2, d_C[p], (int)tile_rows, (int)tile_cols, (int)tile_rows, k_stream);
            CUDA_CHECK(cudaEventRecord(ker_done_evt[p], k_stream));

            CUDA_CHECK(cudaStreamWaitEvent(copy_stream, ker_done_evt[p], 0));
            size_t bytes = tile_rows * (size_t)tile_cols * sizeof(float);
            CUDA_CHECK(cudaMemcpyAsync(h_tile[p], d_C[p], bytes, cudaMemcpyDeviceToHost, copy_stream));
            CUDA_CHECK(cudaEventRecord(d2h_done_evt[p], copy_stream));
    ...
    CUDA_CHECK(cudaFree(d_C[0]));
    CUDA_CHECK(cudaFree(d_C[1]));
    cudaFree(d_db_T);
    cudaFree(d_row_norm2_all);
    cudaFree(d_db_vectors);

    CUDA_CHECK(cudaFreeHost(h_tile[0]));
    CUDA_CHECK(cudaFreeHost(h_tile[1]));
    CUDA_CHECK(cudaFreeHost(flat_data));

    CUDA_CHECK(cudaEventDestroy(up_evt));
    for (int p = 0; p < 2; ++p) {
        CUDA_CHECK(cudaEventDestroy(ker_done_evt[p]));
        CUDA_CHECK(cudaEventDestroy(d2h_done_evt[p]));
    }
    CUDA_CHECK(cudaStreamDestroy(k_stream));
