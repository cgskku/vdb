...
void launch_compute_row_norm2_all(const float* d_src, float* d_out, int N, int D, int TPB, cudaStream_t stream) {
    int blocks = (N + TPB - 1) / TPB;
    row_norm2_kernel<<<blocks, TPB, 0, stream>>>(d_src, d_out, N, D);
}
...
void launch_transpose_rowmajor_to_colmajor(const float* d_src, float* d_dst, int N, int D, cudaStream_t stream) {
    dim3 block(TILE_DIM, BLOCK_ROWS);
    dim3 grid((D + TILE_DIM - 1) / TILE_DIM, (N + TILE_DIM - 1) / TILE_DIM);
    transpose_row2col_kernel<<<grid, block, 0, stream>>>(d_src, d_dst, N, D);
}
...
void launch_fuse_norms_and_dot_to_dist2_colmajor(const float* d_row2, const float* d_col2, float* d_C, int M, int Ncol, int ldc, cudaStream_t stream) {
    dim3 block(32, 8);
    dim3 grid((Ncol + block.x - 1) / block.x, (M    + block.y - 1) / block.y);
    fuse_norms_dot2dist2_colmajor_kernel<<<grid, block, 0, stream>>>(d_row2, d_col2, d_C, M, Ncol, ldc);
}